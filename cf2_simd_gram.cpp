...
void cf2_simd_gram (const uchar *P, const int m, const uchar *T, const int n, const int sigma, const int k) {
	...
	for(; i < n; i++) {
		...
		if ( (sum = sum128.m128i_u16[0] + sum128.m128i_u16[4]) <= k*2*Q_GRAM ) {
			++v_count;
		}
...
int main(int argc, char *argv[]) {
	...
	while (fread (pattern, CH_SIZE, patt_size, file)==patt_size) {
		t_start(t_run);
		cf2_simd_gram(pattern, patt_size, text, text_size, sigma, K_DIFF);
		t_stop(t_run);
	}
