...
void cf2_skip_simd (const uchar *P, const int m, const uchar *T, const int n, const int sigma, const int k) {
...
int main(int argc, char *argv[]) {
	...
	while (fread (pattern, CH_SIZE, patt_size, file)==patt_size) {
		t_start(t_run);
		cf2_skip_simd(pattern, patt_size, text, text_size, sigma, K_DIFF);
		t_stop(t_run);
	}
